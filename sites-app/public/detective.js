...
function updateHeader() {
  if (!state.room) {
    headerStatus.textContent = "誰是犯人 · 真實發票／虛構案件";
    return;
  }
  const labels = {
    lobby: "等待偵探報到",
  ...
  app.innerHTML = `
    <section class="scene hero-grid">
      <div class="hero-copy">
        ...
        <p class="eyebrow">KID · KAHOOT INVOICE DETECTIVE</p>
        <h1>誰是<em>犯人？</em></h1>
      ...
      <aside class="folder-card" aria-label="案件檔案袋">
        ...
        <figure class="home-detective">
          <figcaption><small>LEAD DETECTIVE</small><strong>本案，由我接手。</strong></figcaption>
          <img src="/home-detective.png" alt="202 獎盃失竊案主辦警探" />
        </figure>
  ...
  app.innerHTML = `
    <section class="scene scene-wide ${actClass(question)}">
      <div class="question-card showcase-card">
        ...
        ${room.isHost ? `<div class="button-row" style="margin-top:20px"><button class="primary-button" data-action="advance">${isClue ? "進入末輪下注" : "行動線看完，開盤下注"}</button></div>` : ""}
      </div>
    </section>
  `;
}
    ...
    <section class="scene scene-wide act-timeline reveal-scene">
      <div class="evidence-layout">
        <article class="reveal-card">
          ...
          ${room.isHost ? `<div class="button-row" style="margin-top:24px"><button class="primary-button" data-action="advance">前往結案頁</button></div>` : ""}
        </article>
    ...
    <section class="scene scene-wide ${actClass(question)} reveal-scene">
      <div class="evidence-layout">
        <article class="reveal-card">
          ...
          ${room.isHost ? `<div class="button-row" style="margin-top:24px"><button class="primary-button" data-action="advance">看排行榜</button></div>` : ""}
        </article>
  ...
  app.innerHTML = `
    <section class="scene">
      <article class="leaderboard-card">
        ...
        ${room.isHost ? `<div class="button-row" style="justify-content:center;margin-top:24px"><button class="primary-button" data-action="advance">下一步</button></div>` : ""}
      </article>
    </section>
  `;
}
    ...
    <section class="scene scene-wide">
      ...
      <div class="result-grid">
        <article class="result-card" id="share-card">
          ...
          <div class="culprit-lockup">${portraitMarkup({ name: results.culprit, avatarUrl: results.culpritAvatar }, "culprit-portrait")}<span>證據命中</span></div>
...
async function shareResult() {
  ...
  if (navigator.share) {
    try { await navigator.share({ title: "誰是犯人｜KID", text, url: caseUrl(state.code) }); return; } catch { /* user cancelled */ }
  }
