...
// Internal demo cast portraits, exported from the team's Jira profiles with consent.
const AVATAR_BY_NAME: Record<string, string> = {
  "邵士銓": "/avatars/robert.png",
  "邱庭": "/avatars/michelle.png",
  "彭聿采": "/avatars/rebecca.png",
  "徐瑞妤": "/avatars/xu-ruiyu.png",
  "何品儒": "/avatars/he-pinru.png",
  "黃俊霖": "/avatars/huang-junlin.png",
};
function avatarOf(name: string) { return AVATAR_BY_NAME[name] ?? null; }
...
function questionView(room: Room, viewerPlayer: Player | null) {
  ...
  if (step.kind === "vote") {
    ...
    return {
      ...
      choices: round.statements.map((s, index) => ({ id: String(index), name: s.player, label: s.stmt, avatarUrl: avatarOf(s.player) })),
      viewerOnStage,
  ...
  return {
    ...
    choices: castOf(c).map((name) => ({ id: name, name, label: name, avatarUrl: avatarOf(name), suspect: room.suspects.includes(name) })),
...
export function publicRoom(room: Room, viewer: { playerKey?: string | null; hostKey?: string | null } = {}) {
  ...
  return {
    ...
    ...(room.phase === "lobby" ? {
      identities: cast.map((name) => ({ name, avatarUrl: avatarOf(name), claimedBy: room.players.find((player) => player.identity === name)?.nickname ?? null })),
    ...
    results: room.phase === "results" && room.verdict ? {
      ...
      culpritAvatar: avatarOf(room.verdict.owner),
